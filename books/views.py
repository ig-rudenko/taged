...
def create(request):
    ...
    if request.method == 'POST':
        ...
        if book_form.is_valid() and request.FILES.get('book_file'):
            ...
            res = elasticsearch_control.create_post(es, 'books', {
            ...
            if res and res.get("_id"):
                ...
                if file_format == 'pdf':
                    ...
                    shutil.copyfile(f'{book_path}/{file_name}', f'{book_path}/preview.png')
                else:
                    shutil.copyfile(f'{sys.path[0]}/static/images/books/none_preview.png', f'{book_path}/preview.png')
...
def update(request, book_id):
    ...
    if request.method == 'POST':
        ...
        if book_form.is_valid():
            es = connect_elasticsearch()
            res = elasticsearch_control.update_post(es, 'books', {
                'title': book_form.cleaned_data['title'],
                'author': book_form.cleaned_data['author'],
                'year': str(book_form.cleaned_data['year'] or ''),
                'about': book_form.cleaned_data['about'],
...
def delete(request, book_id):
    ...
    if request.method == 'POST':
        ...
        # Ищем книгу по её ID
        post = es.search(index='books', _source=['_id'], query={
            "simple_query_string": {
                "query": book_id,
                "fields": ['_id']
            }
...
def about_book(request, book_id):
    ...
    try:
        res = es.get(index='books', id=book_id)['_source']  # Получаем запись по ID
        res['published_at'] = datetime.strptime(res['published_at'], '%Y-%m-%dT%H:%M:%S.%f')
        res['id'] = book_id
        return render(request, 'books/about_book.html', {'book': res, 'user': request.user})
    except elasticsearch.exceptions.NotFoundError:
        print('ID not exist')
    return HttpResponseNotFound()
...
def all_books(request):
    ...
    if query and search_form.is_valid():
        ...
        result = es.search(
            size=100, index='books', _source=['title', 'year', 'author'], query=query,
        ...
        if result:
            ...
            for b in result['hits']['hits']:
                res_books.append(
                    dict(b['_source'], **{'id': b['_id']})
                )
    else:
        search_form.is_valid()
        es = connect_elasticsearch()
        res_books = elasticsearch_control.get_last_published(es, index='books', limit=100)
